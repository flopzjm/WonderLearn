import 'package:flutter/material.dart';

class PatternsScreen extends StatefulWidget {
  const PatternsScreen({super.key});

  @override
  State<PatternsScreen> createState() => _PatternsScreenState();
}

class _PatternsScreenState extends State<PatternsScreen> {
  int _currentPatternIndex = 0;
  int _score = 0;
  bool _showFeedback = false;
  String _feedbackMessage = '';

  final List<Map<String, dynamic>> _patterns = [
    {
      'name': 'ABAB',
      'sequence': ['🔴', '🔵', '🔴', '🔵', '?'],
      'options': ['🔴', '🔵', '🟢'],
      'correct': '🔴',
    },
    {
      'name': 'AAB',
      'sequence': ['🟡', '🟡', '🟢', '🟡', '🟡', '?'],
      'options': ['🟡', '🟢', '🔵'],
      'correct': '🟢',
    },
    {
      'name': 'ABAB',
      'sequence': ['⭐', '🌙', '⭐', '🌙', '?'],
      'options': ['⭐', '🌙', '☀️'],
      'correct': '⭐',
    },
    {
      'name': 'AAB',
      'sequence': ['🐱', '🐱', '🐶', '🐱', '🐱', '?'],
      'options': ['🐱', '🐶', '🐰'],
      'correct': '🐶',
    },
  ];

  String? _selectedOption;
  bool _isCorrect = false;

  void _checkAnswer(String selected) {
    final pattern = _patterns[_currentPatternIndex];
    setState(() {
      _selectedOption = selected;
      _isCorrect = selected == pattern['correct'];
      _showFeedback = true;
      if (_isCorrect) _score++;
    });

    Future.delayed(const Duration(seconds: 1), () {
      if (mounted) {
        setState(() {
          _showFeedback = false;
          _selectedOption = null;
          if (_currentPatternIndex < _patterns.length - 1) {
            _currentPatternIndex++;
          } else {
            _showResults();
          }
        });
      }
    });
  }

  void _showResults() {
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (context) => AlertDialog(
        title: const Text('¡Patrones completados! 🔄'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Text('Puntuación: $_score/${_patterns.length}'),
            const SizedBox(height: 8),
            Text('Estrellas: ${"⭐" * (_score >= 3 ? 3 : _score >= 2 ? 2 : 1)}'),
          ],
        ),
        actions: [
          TextButton(
            onPressed: () {
              Navigator.pop(context);
              Navigator.pop(context);
            },
            child: const Text('Volver al mapa'),
          ),
        ],
      ),
    );
  }

  @override
  Widget build(BuildContext context) {
    final pattern = _patterns[_currentPatternIndex];

    return Scaffold(
      body: Container(
        decoration: const BoxDecoration(
          gradient: LinearGradient(
            begin: Alignment.topCenter,
            end: Alignment.bottomCenter,
            colors: [Color(0xFF9CA3AF), Color(0xFFFFF8E1)],
          ),
        ),
        child: SafeArea(
          child: Column(
            children: [
              // Header
              Padding(
                padding: const EdgeInsets.all(16),
                child: Row(
                  children: [
                    IconButton(
                      icon: const Icon(Icons.close, color: Colors.white),
                      onPressed: () => Navigator.pop(context),
                    ),
                    Expanded(
                      child: LinearProgressIndicator(
                        value: (_currentPatternIndex + 1) / _patterns.length,
                        backgroundColor: Colors.white.withOpacity(0.3),
                        valueColor: const AlwaysStoppedAnimation<Color>(Color(0xFFFFD93D)),
                      ),
                    ),
                    const SizedBox(width: 16),
                    Text(
                      '$_score/${_patterns.length}',
                      style: const TextStyle(
                        fontSize: 18,
                        fontWeight: FontWeight.bold,
                        color: Colors.white,
                      ),
                    ),
                  ],
                ),
              ),

              // Pattern display
              Expanded(
                child: Center(
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text(
                        'Patrón: ${pattern['name']}',
                        style: const TextStyle(
                          fontSize: 20,
                          color: Colors.white,
                        ),
                      ),
                      const SizedBox(height: 16),
                      const Text(
                        '¿Qué sigue?',
                        style: TextStyle(
                          fontSize: 24,
                          fontWeight: FontWeight.bold,
                          color: Colors.white,
                        ),
                      ),
                      const SizedBox(height: 32),
                      Container(
                        padding: const EdgeInsets.all(24),
                        decoration: BoxDecoration(
                          color: Colors.white,
                          borderRadius: BorderRadius.circular(16),
                          boxShadow: [
                            BoxShadow(
                              color: Colors.black.withOpacity(0.1),
                              blurRadius: 10,
                              offset: const Offset(0, 5),
                            ),
                          ],
                        ),
                        child: Row(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: (pattern['sequence'] as List).map((item) {
                            final isQuestion = item == '?';
                            return Container(
                              width: isQuestion ? 60 : 50,
                              height: isQuestion ? 60 : 50,
                              margin: const EdgeInsets.symmetric(horizontal: 4),
                              decoration: BoxDecoration(
                                color: isQuestion
                                    ? const Color(0xFFFFD93D).withOpacity(0.3)
                                    : Colors.grey.shade100,
                                borderRadius: BorderRadius.circular(8),
                                border: isQuestion
                                    ? Border.all(
                                        color: const Color(0xFFFFD93D),
                                        width: 2,
                                      )
                                    : null,
                              ),
                              child: Center(
                                child: isQuestion
                                    ? const Icon(Icons.question_mark, color: Color(0xFFFFD93D))
                                    : Text(
                                        item,
                                        style: const TextStyle(fontSize: 24),
                                      ),
                              ),
                            );
                          }).toList(),
                        ),
                      ),
                    ],
                  ),
                ),
              ),

              // Answer options
              Padding(
                padding: const EdgeInsets.all(24),
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                  children: (pattern['options'] as List).map((option) {
                    final isSelected = _selectedOption == option;
                    final isCorrectOption = option == pattern['correct'];
                    Color bgColor;
                    
                    if (_showFeedback) {
                      if (isCorrectOption) {
                        bgColor = const Color(0xFF6BCB77);
                      } else if (isSelected && !_isCorrect) {
                        bgColor = const Color(0xFFFF6B6B);
                      } else {
                        bgColor = Colors.white;
                      }
                    } else {
                      bgColor = Colors.white;
                    }

                    return GestureDetector(
                      onTap: _showFeedback ? null : () => _checkAnswer(option),
                      child: AnimatedContainer(
                        duration: const Duration(milliseconds: 200),
                        width: 80,
                        height: 80,
                        decoration: BoxDecoration(
                          color: bgColor,
                          borderRadius: BorderRadius.circular(12),
                          boxShadow: [
                            BoxShadow(
                              color: Colors.black.withOpacity(0.1),
                              blurRadius: 5,
                              offset: const Offset(0, 3),
                            ),
                          ],
                        ),
                        child: Center(
                          child: Text(
                            option,
                            style: const TextStyle(fontSize: 32),
                          ),
                        ),
                      ),
                    );
                  }).toList(),
                ),
              ),

              // Feedback
              if (_showFeedback)
                Container(
                  padding: const EdgeInsets.all(16),
                  color: _isCorrect ? const Color(0xFF6BCB77) : const Color(0xFFFF6B6B),
                  child: Text(
                    _isCorrect ? '¡Correcto! 🎉' : '¡Intenta de nuevo!',
                    style: const TextStyle(
                      fontSize: 18,
                      fontWeight: FontWeight.bold,
                      color: Colors.white,
                    ),
                  ),
                ),
            ],
          ),
        ),
      ),
    );
  }
}
